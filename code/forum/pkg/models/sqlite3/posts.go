...
func (m *PostModel) Insert(user_id int, title, body string, categories []int) (int, error) {
	post_stmt := `
	INSERT INTO post (user_id, title, body, created)
	...
	result, err := tx.Exec(post_stmt, user_id, title, body)
	if err != nil {
		tx.Rollback()
		return 0, err
	}
...
func (m *PostModel) Get(post_id int) (*models.Post, error) {
	post_stmt := `
	SELECT post.post_id, post.title, post.body, post.created, useraccount.username FROM post
	...
	err := row.Scan(&p.ID, &p.Title, &p.Body, &p.Created, &p.Author)
